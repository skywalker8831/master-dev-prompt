...
def test_process_transcript_skips_output_on_nonzero_returncode(tmp_path):
    ...
    assert (outputs_dir / "meeting.log").read_text() == "script error\n"
...
def test_main_exits_when_transcripts_dir_missing(tmp_path):
    ...
    with patch("sys.argv", ["watcher.py", "--transcripts", str(missing), "--outputs", str(outputs)]):
...
def test_main_starts_and_stops_observer(tmp_path):
    ...
    with patch("sys.argv", ["watcher.py", "--transcripts", str(transcripts_dir), "--outputs", str(outputs_dir)]):
        with patch("watcher.Observer", return_value=mock_observer):
            with patch("watcher.time.sleep", side_effect=KeyboardInterrupt):
                main()
